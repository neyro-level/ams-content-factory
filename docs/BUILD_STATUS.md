...
Required inputs not present in this workspace: Timeweb Cloud PostgreSQL cluster/connection data, enabled `pgvector`, TLS policy/certificate settings and database backup schedule; server IP/hostname, SSH user and port, server OS/Docker confirmation, final domain/TLS arrangement, S3 endpoint/bucket/credentials, Instagram and VK app credentials, and live OpenAI/HeyGen/Motion credentials. Production deployment additionally requires explicit infrastructure confirmation from the owner.
