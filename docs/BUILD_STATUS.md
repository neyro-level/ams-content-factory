...
- **Current Task:** W3.5 — private SourceCraft inception and CI setup.
...
- **Remote:** private SourceCraft repository `integrator-p/ams-content-factory` created; canonical origin, branch policy and verify CI are pending the initial push.
- **External blockers:** none. Docker Desktop, WSL 2 and local PostgreSQL with pgvector are healthy.
- **Next step:** create private SourceCraft repository, set canonical `origin`, push `main` and add verify CI. No production deploy.
